...
print('---------- constant ----------')
k.run_measurement([1, 4, 2, 3], nb_stack=1, injection_duration=0.2, duty_cycle=0.7, strategy='constant', vab_req=15)
print('---------- full_constant ----------')
...
try:
    k.remove_data()
    k.create_sequence(8, params=[('wenner', 3)], fw_in_csv=True)
    k.run_sequence(vab_req=5., duty_cycle=1, injection_duration=0.1, strategy='full_constant')
